...
void Screen::drawWidgets() {
    ...
    if (elapsed > 0.5f) {
        /* Draw tooltips */
        const ref<Widget> widget = findWidget(mMousePos);
...
bool Screen::cursorPosCallbackEvent(double x, double y) {
    ...
    try {
        ...
        if (!mDragActive) {
            ref<Widget> widget = findWidget(p);
...
bool Screen::mouseButtonCallbackEvent(int button, int action, int modifiers) {
    ...
    try {
        if (mFocusPath.size() > 1) {
            ref<Window> window =
                dynamic_pointer_cast<Window>(mFocusPath[mFocusPath.size() - 2]);
...
void Screen::updateFocus(ref<Widget> widget) {
    ...
    if (window)
        moveWindowToFront(static_pointer_cast<Window>(window));
}

void Screen::disposeWindow(ref<Window> window) {
...
void Screen::moveWindowToFront(ref<Window> window) {
    ...
    do {
        ...
        changed = false;
        for (size_t index = 0; index < mChildren.size(); ++index) {
            ref<Popup> pw = dynamic_pointer_cast<Popup>(mChildren[index]);
