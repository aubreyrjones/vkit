...
Label::Label(ref<Widget> parent, const std::string &caption, const std::string &font, int fontSize)
    : Widget(parent), mCaption(caption), mFont(font) {
    mFontSize = fontSize < 0 ? mTheme->mStandardFontSize : fontSize;
    mColor = mTheme->mTextColor;
}

Vector2i Label::preferredSize(NVGcontext *ctx) {
    if (mCaption == "")
    ...
    if (mFixedSize.x() > 0) {
        ...
        return Vector2i(nvgTextBounds(ctx, 0, 0, mCaption.c_str(), nullptr, nullptr), mTheme->mStandardFontSize);
    }
}
