...
TextBox::TextBox(ref<Widget> parent,const std::string &value)
    : Widget(parent),
      mEditable(false),
      mCommitted(true),
      mValue(value),
...
Vector2i TextBox::preferredSize(NVGcontext *ctx) {
...
bool TextBox::copySelection() {
    if (mSelectionPos > -1) {
        ref<Screen> sc = dynamic_pointer_cast<Screen>(this->window()->parent());
