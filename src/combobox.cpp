...
ComboBox::ComboBox(ref<Widget> parent) : PopupButton(parent), mSelectedIndex(0) {
}

ComboBox::ComboBox(ref<Widget> parent, const std::vector<std::string> &items)
    : PopupButton(parent), mSelectedIndex(0) {
    setItems(items);
}

ComboBox::ComboBox(ref<Widget> parent, const std::vector<std::string> &items, const std::vector<std::string> &itemsShort)
    : PopupButton(parent), mSelectedIndex(0) {
...
void ComboBox::setSelectedIndex(int idx) {
    if (mItemsShort.empty())
        return;
    const std::vector<ref<Widget> > &children = popup()->children();
    static_pointer_cast<Button>(children[mSelectedIndex])->setPushed(false);
	static_pointer_cast<Button>(children[idx])->setPushed(true);
    mSelectedIndex = idx;
...
void ComboBox::setItems(const std::vector<std::string> &items, const std::vector<std::string> &itemsShort) {
    ...
    mPopup->setLayout(makeref<GroupLayout>(10));
    int index = 0;
