...
public:
    ...
    ref<Window> addWindow(const Vector2i &pos,
                         const std::string &title = "Untitled") {
        assert(mScreen);
        mWindow = ref<Window>(new Window(mScreen, title));
        mLayout = ref<Layout>(new AdvancedGridLayout({10, 0, 10, 0}, {}));
...
public:
    FormWidget(ref<Widget> p) : CheckBox(p, "") { setFixedWidth(20); }
...
public:
    FormWidget(ref<Widget> p) : ComboBox(p) { }
...
public:
    FormWidget(ref<Widget> p) : IntBox<T>(p) { this->setAlignment(TextBox::Alignment::Right); }
};

template <typename T> class FormWidget<T, typename std::is_floating_point<T>::type> : public FloatBox<T> {
public:
    FormWidget(ref<Widget> p) : FloatBox<T>(p) { this->setAlignment(TextBox::Alignment::Right); }
};

template <> class FormWidget<std::string, std::true_type> : public TextBox {
public:
    FormWidget(ref<Widget> p) : TextBox(p) { setAlignment(TextBox::Alignment::Left); }
...
public:
    FormWidget(ref<Widget> p) : ColorPicker(p) { }
